...
#define SD_DATA_R_TIMEOUT             ((uint32_t)((48000000U / (SD_CLK_DIV_TRAN + 2U) / 1000U) * 100U) + 1000) // Data read timeout is 100ms
...
typedef struct {
    ...
    uint16_t OID;   // OEM / Apllication ID
    uint64_t PNM: 40;   // Product name
    uint8_t PRV;    // Product revision
    uint32_t PSN;   // Product serial number
    uint8_t res: 4;
    uint16_t MDT: 12;  // Manufactoring date
